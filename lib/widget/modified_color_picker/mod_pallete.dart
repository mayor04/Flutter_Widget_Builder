...
import 'package:flutter_widget_builder/widget/modified_color_picker/mod_utils.dart';
...
class _SliderLayout extends MultiChildLayoutDelegate {
  ...
  void performLayout(Size size) {
    ...
    positionChild(track, Offset(0, size.height * 0.4));
    layoutChild(
      thumb,
      BoxConstraints.tightFor(width: 5.0, height: size.height / 3),
    );
    positionChild(thumb, Offset(0.0, size.height * 0.46));
    layoutChild(
      gestureContainer,
...
class ColorPickerSlider extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints box) {
      double thumbOffset = 7.0;
      Color thumbColor;
      switch (trackType) {
        case TrackType.hue:
          thumbOffset += (box.maxWidth - 13.0) * hsvColor.hue / 360;
        ...
        case TrackType.blue:
          ...
          break;
        case TrackType.alpha:
          thumbOffset += (box.maxWidth - 13.0) * hsvColor.toColor().opacity;
